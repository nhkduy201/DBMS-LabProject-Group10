...
function doiGiaSP() {
  ...
  btn.addEventListener("click", (e) => {
    let data = {
      masp: document.getElementById("cmsp").value,
      gia: document.getElementById("dgi").value,
    };
      ...
      for (let i = 0; i < data.res.length; ++i) {
        ...
        btn.addEventListener("click", (e) => {
          ...
          fetch(`http://127.0.0.1:8000/lay-thong-tin/`, {
            ...
            .then((json) => {
              createAlert(
                "Tổng thu nhập hiện tại: " + json.tongtien,
                "Số lượng đơn hàng đã nhận: " + json.sldonhang
              );
            });
        });
        td.appendChild(btn);
    ...
    .then((data) => {
      ...
      for (let i = 0; i < data.res.length; ++i) {
        ...
        tbody.insertAdjacentHTML(
          ...
          `<td>${row.MaDH}</td>
        <td>${row.PhiVC}</td>
        <td>${row.MaTaiXe == null ? "Chưa có" : row.MaTaiXe}</td>`
        );
      }
    });
}
